...
export default function ChatWidget() {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 z-50 bg-white dark:bg-zinc-950 border-2 border-black dark:border-white px-6 py-4 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors duration-200 flex items-center gap-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
      >
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 bg-white dark:bg-zinc-950 border-2 border-black dark:border-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] ${isMinimized ? 'w-80' : 'w-96'} transition-all duration-300`}>
      {/* Header */}
      <div className="bg-black text-white dark:bg-white dark:text-black p-4 flex items-center justify-between">
      ...
      {!isMinimized && (
        <>
          <div className="h-96 overflow-y-auto p-4 space-y-4 bg-white dark:bg-zinc-950">
            {messages.map((msg, idx) => (
              <div key={idx}>
                <div
                  className={`max-w-[85%] p-3 border-2 border-black dark:border-white ${
                    msg.type === 'bot'
                      ? 'bg-blue-600 text-white mr-auto'
                      : 'bg-gray-100 dark:bg-zinc-800 text-black dark:text-white ml-auto'
                  }`}
                >
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {msg.options.map((opt, i) => (
                      <button
                        ...
                        className="text-xs font-mono border-2 border-black dark:border-white px-3 py-1.5 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors"
                      >
                        {opt}
                      </button>
                    ))}
          ...
          {/* Input */}
          <form onSubmit={handleSubmit} className="p-4 border-t-2 border-black dark:border-white bg-white dark:bg-zinc-950">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border-2 border-black dark:border-white focus:outline-none focus:ring-2 focus:ring-blue-600"
              />
              <button
                type="submit"
                disabled={!input.trim()}
                className="bg-black text-white dark:bg-white dark:text-black p-2 border-2 border-black dark:border-white hover:bg-blue-600 dark:hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                <Send size={20} />
              </button>
            </div>
